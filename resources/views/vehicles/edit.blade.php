...
@section('page-subtitle', 'Edit informasi kendaraan')
        ...
        <div class="card-body">
            <form action="{{ route('vehicles.update', $vehicle) }}" method="POST">
                    ...
                    <div class="card-body">
                        <div class="row mb-3">
                            <div class="col-md-4">
                                ...
                                <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                                    ...
                                    <option value="motorcycle" {{ old('type', $vehicle->type) == 'motorcycle' ? 'selected' : '' }}>Sepeda Motor</option>
                                    <option value="electric_bike" {{ old('type', $vehicle->type) == 'electric_bike' ? 'selected' : '' }}>Sepeda Listrik</option>
                                </select>
                                @error('type')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                        ...
                        <div class="row mb-3">
                            <div class="col-md-3">
                                ...
                                <input type="number" class="form-control @error('year') is-invalid @enderror" id="year" name="year" value="{{ old('year', $vehicle->year) }}" min="1900" max="{{ date('Y') + 1 }}" required>
                                @error('year')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="text" class="form-control text-uppercase @error('license_plate') is-invalid @enderror" id="license_plate" name="license_plate" value="{{ old('license_plate', $vehicle->license_plate) }}" required>
                                @error('license_plate')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
                            ...
                            <div class="col-md-3">
                                ...
                                <div class="color-preview rounded" style="height: 38px; border: 1px solid #ced4da; background-color: {{ old('color', $vehicle->color) ?: 'transparent' }}"></div>
                            </div>
                        </div>
                        
                        <div class="mb-3">
                            <label for="notes" class="form-label">Catatan</label>
                            <textarea class="form-control @error('notes') is-invalid @enderror" id="notes" name="notes" rows="3" placeholder="Catatan tambahan tentang kendaraan...">{{ old('notes', $vehicle->notes) }}</textarea>
                            @error('notes')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                ...
                <div class="d-flex justify-content-between">
                    ...
                    <div>
                        <a href="{{ route('vehicles.show', $vehicle) }}" class="btn btn-info me-2">
...
<script>
    $(document).ready(function() {
        ...
        // Trigger change event if there's a selected customer (show initial info)
        if ($('#customer_id').val()) {
